...
class PasswordGenerator:
    def __init__(self, lenght: int):
        ...
        )  # No symbols due to passwords cannot contain symbols in geometry dash.
...
def main(
    password: str = typer.Argument(
        None, help="Password you want to generate hashes for."
    )
):
    if not password:
        try:
            ...
            print(
                f"No lenght specified or lenght is not valid ({e}), proceeding with 16 characters."
            )
            pw_lenght = 16

        generator = PasswordGenerator(pw_lenght)
        print("Generating...")
        password = generator.gen_pw
    ...
    print(
        f"Generated GJP1 hash (password field): {hasher.gen_gjp1}\nGenerated GJP2 hash (gjp2 field): {hasher.gen_gjp2}"
    )
